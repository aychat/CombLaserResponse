...
class FrequencyComb(PolarizationTerms):
    ...
    def pol2_basis_change_freq2comb(self):
        ...
        print "shape=", self.pol2_comb_matrix.shape
...
if __name__ == '__main__':
    ...
    het_fields = [
        np.dot(ensemble.heterodyne_fields_comb_basis(i), ensemble.freq2comb_basis.T)[ensemble.N_molecules][:]
        for i in range(ensemble.N_molecules)
    ]
    for i in range(ensemble.N_molecules):
        het_fields[i] /= het_fields[i].max()

    plt.figure()
    [(
        plt.subplot(331+i),
        plt.plot(ensemble.frequency, ensemble.pol2_freq_matrix[:, i], 'r'),
        plt.xlabel("Frequency (in fs)"),
        plt.ylabel("Polarization $P^{(2)}(\\omega)$ \n (in arbitrary units)"),
    ) for i in range(ensemble.N_molecules)]
    ...
    [(
        ...
        plt.xlabel("Frequency (in fs)"),
        plt.ylabel("Heterodyne fields E_{het}(\\omega)$ \n (in arbitrary units)"),
    ) for i in range(ensemble.N_molecules)]

    [(
        plt.subplot(337 + i),
        plt.plot(ensemble.frequency,
                 ensemble.heterodyne_fields_frequency_basis(i).sum(axis=0), 'b'
                 ),
        plt.xlabel("Frequency (in fs)"),
        plt.ylabel("Heterodyne fields E_{het}(\\omega)$ \n (in arbitrary units)"),
    ) for i in range(ensemble.N_molecules)]
    ...
    plt.figure()
    plt.plot(ensemble.frequency, ensemble.field1/ensemble.field1.max(), 'y')
    plt.plot(ensemble.frequency, ensemble.field2/ensemble.field2.max(), 'g')
    plt.plot(ensemble.frequency, ensemble.freq2comb_basis/ensemble.freq2comb_basis.max(), 'c')
    plt.plot(ensemble.frequency, ensemble.pol2_freq_matrix[:, 0]/ensemble.pol2_freq_matrix[:, 0].max(), 'b')
    plt.plot(ensemble.frequency, ensemble.pol2_freq_matrix[:, 1]/ensemble.pol2_freq_matrix[:, 1].max(), 'k')
    plt.plot(ensemble.frequency, ensemble.pol2_freq_matrix[:, 2]/ensemble.pol2_freq_matrix[:, 2].max(), 'm')
    plt.xlabel("Frequency (in fs)")
    plt.ylabel("Comb fields and molecular polarizations \n (in arbitrary units)")
    # A = np.abs(np.asarray([ensemble.heterodyne_fields_comb_basis(i)[1] for i in range(ensemble.N_molecules)]))
    # print np.asarray([A[i, :]/A[i, :].min() for i in range(3)])

    # plt.figure()
    # plt.plot(ensemble.frequency, 3 * ensemble.field1 / ensemble.field1.max(), 'k')
    # plt.plot(ensemble.frequency, 3 * ensemble.field2 / ensemble.field2.max(), 'r')
    # plt.xlabel("Frequency in fs$^{-1}$")
    # plt.ylabel("Electric field (arb. units)")

    # plt.figure()
    # plt.subplot(321)
    # plt.plot(ensemble.frequency, np.dot(ensemble.freq2comb_basis, ensemble.pol2_comb_matrix)[:, 0])
    # plt.subplot(323)
    # plt.plot(ensemble.frequency, np.dot(ensemble.freq2comb_basis, ensemble.pol2_comb_matrix)[:, 1])
    # plt.subplot(325)
    # plt.plot(ensemble.frequency, np.dot(ensemble.freq2comb_basis, ensemble.pol2_comb_matrix)[:, 2])
    # plt.subplot(121)
    # for i in range(2*ensemble.N_comb):
    #     plt.plot(ensemble.freq2comb_basis[:, i])
    # plt.subplot(122)
    # plt.plot(ensemble.frequency, ensemble.pol2_freq_matrix[:, 0])
    plt.show()